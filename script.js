...
/* harmony import */ var _modules_forms__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/forms */ "./src/js/modules/forms.js");
 ...
 // Функция modals запускается только после загрузки страницы
...
var forms = function forms() {
  var form = document.querySelectorAll('form'),
      ...
      phoneInputs = document.querySelectorAll('input[name="user_phone"]'); // Валидация формы инпутов с телефоном

  phoneInputs.forEach(function (item) {
    item.addEventListener('input', function () {
      // Заменяем все буквы пустым местом
      item.value = item.value.replace(/\D/, '');
    });
  }); // Сообщения которые будут показываться пользователю в том или ином случае
  ...
  form.forEach(function (item) {
    // Отправляем каждую форму
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // Добавляем новый элемент на страницу, для вывода сообщения о ходе статуса формы

      var statusMessage = document.createElement('div'); // Добавляем стили к статусу

      statusMessage.classList.add('status'); // Помещаем в конец формы

      item.appendChild(statusMessage); // Собираем все данные, которые есть у нас в форме

      var formData = new FormData(item); // Проверка состаяния отправки формы
...
var modals = function modals() {
  //Общий алгоритм, принимает в себя различные аргументы
  function bindModal(trigerSelector, modalSelector, closeSelector) {
    ...
    triger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        modal.style.display = 'block'; //Запрещает скрол при активной табе
  ...
  } // Фукция для выведения модального окна чере 60 секунд
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  var header = document.querySelector(headerSelector),
      ...
      content = document.querySelectorAll(contentSelector); // Скрытие контента

  function hideTabContent() {
    // Перебирается каждый элемент
    content.forEach(function (item) {
      // Каждый элемент скрывается
      item.style.display = 'none';
    }); // У каждого элемента удаляется класс активности
  ...
  hideTabContent();
  showTabContent(); // Фукнция прослушивания табов

  header.addEventListener('click', function (e) {
    var target = e.target; // Проверка что пользователь нажал именно в один из табов

    if ( // Проверяется, есть ли вообще target
    target && (target.classList.contains(tabSelector.replace(/\./, "")) || // Проверяем так же родительский класс и заменяем точку в начале
